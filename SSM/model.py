...
class SSM(Base):
    def __init__(self, args):
        ...
        for i in range(self.num_states):
            ...
            if len(args.device_ids) > 0:
                prior = nn.DataParallel(prior, device_ids=args.device_ids)
                posterior = nn.DataParallel(posterior, device_ids=args.device_ids)
                encoder = nn.DataParallel(encoder, device_ids=args.device_ids)
                decoder = nn.DataParallel(decoder, device_ids=args.device_ids)
    ...
    def forward(self, x_0, x, a, prior_sample=True, return_x=False):
        ...
        for t in range(_T):
            ...
            for i in range(self.num_states):
                ...
                s_prev = s_prevs[i]
                a_list = [a_t] + s_ts[-1:]  # use one hierarchy upper state

                q = Normal(*self.posteriors[i](s_prev, h_t, a_list))
                p = Normal(*self.priors[i](s_prev, a_list))
                s_losss[i] = torch.sum(kl_divergence(q, p), dim=[1]).mean()  # (q,p)! Not (p,q)!
                ...
                x_losss[i] += - torch.sum(decoder_dist.log_prob(x_t), dim=[1,2,3]).mean()
    ...
    def sample_s_0(self, x_0):
        _B = x_0.size(0)
        s_prevs = [torch.zeros(_B, s_dim).to(self.device) for s_dim in self.s_dims]
        s_ts = []
        for i in range(self.num_states):
            h_t = self.encoders[i](x_0)
            s_prev = s_prevs[i]
            a_list = [torch.zeros(_B, d).to(self.device) for d in [self.a_dim] + self.s_dims[i-1:i]]
            with torch.no_grad():
                q = Normal(*self.posteriors[i](s_prev, h_t, a_list))
                s_t = q.mean
            s_ts.append(s_t)
